...
src/tuuuui/
  ...
  launcher.py          tuuuui-tmux: tmux 起動 + 2ペイン構成の一括ランチャー
  app.py               TuuuuiApp: 2 ペイン配置・C-x プレフィックス・モード切替
  app.tcss             レイアウト用スタイル
  core/
...
cd tuuuui
python3 -m venv .venv
.venv/bin/pip install -e .          # シンタックスハイライトの依存も同梱
```
...
### tmux + tuuuui + ワークスペースを 1 コマンドで起動

専用ランチャー `tuuuui-tmux` で、tmux セッションの作成・左に tuuuui・右に AI CLI の
起動・tuuuui へのフォーカスまでを一括で行えます。

**前提**

- **tmux** がインストール済み（`brew install tmux` / `apt install tmux` など）。
- 右ペインで動かす **AI CLI** が PATH 上にあること（例: `claude`、GitHub Copilot CLI
  の `copilot` など）。
- **tuuuui** がインストール済み（上記「インストール」。`tuuuui-tmux` も同時に入ります）。
  venv を使う場合は `.venv/bin/` を PATH に通すか、`.venv/bin/tuuuui-tmux` と直接叩く。

**使い方**

```bash
# カレントディレクトリ。左 = tuuuui、右 = claude（既定）
tuuuui-tmux

# ディレクトリ指定 + 右ペインを copilot に
...
- 右ペインの既定コマンドは環境変数 `TUUUUI_WORKSPACE_CMD` でも指定できます。
- すでに tmux 内で実行した場合は、入れ子セッションを作らず現在のウィンドウを分割します
  （`tuuuui --tmux` に委譲）。

**追加ツール無しの素の tmux ワンライナー**（`tuuuui` と `claude` が PATH 上にある前提）:
