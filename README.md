...
```
┌──────────────┬───────────────────────────┬──────────────────┐
│  Filer       │  Git view  /  File view   │  Workspace       │
│  (file tree) │                           │  (tmux pane:     │
│  + buffers   │  emacs editor /           │   claude code /  │
│  (C-x b)     │  git log + diff           │   copilot cli)   │
└──────────────┴───────────────────────────┴──────────────────┘
```
...
- **右 — ワークスペース**: `claude code` や GitHub Copilot CLI などの対話的 CLI を、
  PTY 埋め込みではなく **tmux の分割ペイン**として隣に起動します（最も安定）。
...
### Git ビュー（中央）
- 上下 2 ペイン構成。
- **上**: `git log` を 1 コミット 1 行で表示、スクロール可能。**一定間隔で自動更新**
  され、新しいコミットが自動で反映されます（選択中コミットとスクロール位置は維持）。
- **下**: 既定は unstaged の作業ツリー diff。上でコミットを選ぶとそのコミットの
  diff に切り替わります。diff は自動更新せず、**⟳ ボタン**または **`C-r`** で
  明示的にリロード（編集中の差分確認に便利）。追加=緑 / 削除=赤 / hunk=シアンで色分け。
